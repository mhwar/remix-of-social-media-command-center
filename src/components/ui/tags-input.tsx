...
export function TagsInput({ value, onChange, placeholder, className }: Props) {
  ...
  return (
    <div
      className={`flex min-h-[42px] flex-wrap items-center gap-1.5 rounded-md border border-input bg-background p-1.5 focus-within:ring-2 focus-within:ring-ring ${className ?? ""}`}
    >
      {value.map((tag, i) => (
        <span
          key={`${tag}-${i}`}
